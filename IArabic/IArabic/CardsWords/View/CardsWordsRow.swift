...
struct CardsWordsRow2: View {
    ...
    @ObservedObject var dataWords: CoreDataViewModel
    
    var body: some View {
        VStack(spacing: 15) {
            SnapCarousel(index: $currentIndex, items: dataWords.saveEntities) { word in
                GeometryReader { proxy in
                    let size = proxy.size
                    
                    CardWordsItem(textTitle: word.translate, textTranslate: word.title, mainImage: word.imageMain, assImage: word.associatImage)
